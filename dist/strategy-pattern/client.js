...
const validator = new context_1.Validator();
// 검증할 샘플 데이터입니다.
const username = '빽다방';
...
validator.setStrategy(new strategies_1.RequiredStrategy());
validator.validate(username);
validator.validate(emptyInput);
// 이메일은 이메일 형식 전략을 사용한다.
validator.setStrategy(new strategies_1.EmailStrategy());
validator.validate(email);
// 비밀번호는 최소 길이 8인 전략을 사용한다.
validator.setStrategy(new strategies_1.MinLengthStrategy(8));
